...
The first plot is unclean mean awc X unclean standard deviation. This are measures for awc without cleaning for naps.
...
First we have the unclean LENA histograms across all subjects. We have AWC, CTC, and CVC. 
Observations:
...
Observations: 
  We saw that every child had at least two contigous segments with the same AWC count. Thus, all children had a mininum drop of 0.
...
We see that they are not correlation at all.
```{r, echo = FALSE, message = FALSE}
...
```
It is very sportatic, and so to see how individual differences in children's dense periods of talk have with respect to their later learning. We then only started to see how many instances of talk there were above each child's respective 70th percentile of AWC. These graphs look as such.
...
Average cluster size was found not to correlate with AWC or number of clusters. 
