...
csvAWVseg.unorderedPercentile<- function(df, percentile, na.rm = TRUE, ...){
  ...
  for (i in seq_along(subject_list)) { 
    ...
    awcPerc<- quantile(df$AWC, probs = c(percentile))
    ctcPerc<- quantile(df$CTC, probs = c(percentile))
    cvcPerc<- quantile(df$CVC, probs = c(percentile))
    ...
    write_csv(cur_df, paste('SubLenaVals/70thpercentile/',subject_list[i], 'SegVals70th.csv', sep=''))
  }
...
#csvAWCseg.unordered(sotCodingSheet16)
#csvAWVseg.unorderedPercentile(sotCodingSheet16, .70)
```
