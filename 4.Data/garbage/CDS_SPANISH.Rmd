...
```{r, echo = FALSE, message = FALSE}
hourlyOrdered16 <- read_csv("Spanish_Dataset/proportionData/hourlyOrdredAwc16Spanish.csv") 
hourlyProp16 <- read_csv("Spanish_Dataset/proportionData/hourlyProportionsSpanish.csv")
segOrdered16 <- read_csv("Spanish_Dataset/proportionData/segOrdredAwc16Spanish.csv") 
segProp16 <- read_csv("Spanish_Dataset/proportionData/segProportionsSpanish.csv") 
clusterValue <- read_csv("Spanish_Dataset/clusterValues/clustersAt0.7with0.3SCspanish.csv")
giniCoefficients <- read_csv("Spanish_Dataset/gini/giniCoefspanish.csv")
outcomeMeasures <- read_csv("Spanish/outcomes_spanish.csv")
spanishDataset<-read_csv("Spanish/analysisTidySpanishDataset.csv")



# spanishMetaData <- read_csv("HABLA_EVALUATION_23Oct2020.csv") %>%
#   rename(SUBJECT = id) %>% 
#   filter(SUBJECT %in% spanishDataset$SUBJECT) %>% 
#   distinct(SUBJECT, L1PRE_hi, L1PRE_mom_ed) %>% 
#   filter(SUBJECT != 436)

# mean(spanishMetaData$L1PRE_hi)
# mean(spanishMetaData$L1PRE_mom_ed)
# 
# 
...
clusterAndDistrubtionMeasuresCDI <- clusterAndDistrubtionMeasures %>% 
  filter(CDI != -9) %>% 
  distinct(SUBJECT, nclusters, avgClustersize, minClusterAWC, maxClusterAWC, avgClusterDensity, unclean_total_hrs, clean_total_hrs, unclean_mean_awc, clean_mean_awc, avgClusterDensityPerHr, gini,avgPropDrop, clusterPerHr, CDI)



clusterAndDistrubtionMeasuresACC <- clusterAndDistrubtionMeasures %>% 
  filter(ACC != -9 ) %>% 
  distinct(SUBJECT, nclusters, avgClustersize, minClusterAWC, maxClusterAWC, avgClusterDensity, unclean_total_hrs, clean_total_hrs, unclean_mean_awc, clean_mean_awc, avgClusterDensityPerHr, gini,avgPropDrop, clusterPerHr, ACC)
  

clusterAndDistrubtionMeasuresRT <- clusterAndDistrubtionMeasures %>% 
  filter(RT >= 0 ) %>% 
   distinct(SUBJECT, nclusters, avgClustersize, minClusterAWC, maxClusterAWC, avgClusterDensity, unclean_total_hrs, clean_total_hrs, unclean_mean_awc, clean_mean_awc, avgClusterDensityPerHr, gini, avgPropDrop, clusterPerHr, RT)
  
# 
# cdiCors <- rcorr(as.matrix(clusterAndDistrubtionMeasuresCDI))
# accCors <- rcorr(as.matrix(clusterAndDistrubtionMeasuresACC))
# rtCors <- rcorr(as.matrix(clusterAndDistrubtionMeasuresRT))
# cdiCors
# accCors
# rtCors
# 
...
# pACC
# pRT
clusterXCDI <- ggplot(clusterAndDistrubtionMeasuresCDI, aes(clusterPerHr, CDI ))+
  geom_point() +
  geom_smooth(method = "lm") +
   ggtitle("Vocabulary") +
...
clusterXACC <- ggplot(clusterAndDistrubtionMeasuresACC, aes(clusterPerHr, ACC))+
  geom_point() +
   geom_smooth(method = "lm")+
  ggtitle("Accuracy") +
  xlab("Clusters/Hour ~ 16 Months") + ylab("Acurracy at 18 Months")

clusterXRT <- ggplot(clusterAndDistrubtionMeasuresRT, aes(clusterPerHr, RT))+
  geom_point() +
   geom_smooth(method = "lm")+
...
rcorr(clusterAndDistrubtionMeasuresRT$clusterPerHr, clusterAndDistrubtionMeasuresRT$RT)
